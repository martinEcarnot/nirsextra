...
adj_NaturaSpec <- function(X) {
  ...
  for (iadj in c(632)) {
    ...
    X[,(iadj+1):(iadj+interpw)] = drop(replicate(interpw,b0)) + t(b)%*%((iadj+1):(iadj+interpw))

    # Compute difference between adjusted and original spectra after gap, and apply it to the rest of the spectrum
    dif=X[,iadj+interpw+1]-(b0+b*(iadj+interpw+1))
    loremp=(iadj+interpw+1):dim(X)[2]
    X[,loremp]=X[,loremp] - drop(replicate(length(loremp),dif))
  }
  return(X)
